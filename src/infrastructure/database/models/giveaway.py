...
class GiveawayEntry(BaseSql, TimestampMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            ")",
            name="ck_giveaway_entry_source_fields",
        ),
    )
    ...
    transaction_payment_id: Mapped[Optional[UUID]] = mapped_column(index=True)
    plan_id: Mapped[int]
    plan_name: Mapped[str] = mapped_column(String(128))
    duration_days: Mapped[int]
    purchase_type: Mapped[Optional[PurchaseType]]
    entry_source: Mapped[GiveawayEntrySource] = mapped_column(
        default=GiveawayEntrySource.AUTO_PURCHASE,
    )
    ...
    user: Mapped[Optional["User"]] = relationship(foreign_keys=[user_telegram_id])
