...
_GIVEAWAY_CODE_ALPHABET = string.ascii_uppercase + string.digits


def generate_giveaway_code(prefix: str) -> str:
    left = "".join(secrets.choice(_GIVEAWAY_CODE_ALPHABET) for _ in range(4))
    right = "".join(secrets.choice(_GIVEAWAY_CODE_ALPHABET) for _ in range(4))
    return f"{prefix}-{left}-{right}"


def mask_giveaway_phone(phone: str) -> str:
...
@dataclass(frozen=True)
class UpdateGiveawayRulesDto:
    campaign_id: int
    rules_text: Optional[str]


class UpdateGiveawayRules(Interactor[UpdateGiveawayRulesDto, GiveawayCampaignDto]):
    required_permission = Permission.REMNASHOP_GIVEAWAY_EDITOR
    ...
    async def _execute(
        self,
        actor: UserDto,
        data: UpdateGiveawayRulesDto,
    ) -> GiveawayCampaignDto:
        rules_text = data.rules_text.strip() if data.rules_text else None
        if rules_text and len(rules_text) > 4000:
            raise ValueError("Giveaway rules are too long")
        async with self.uow:
            campaign = await self.giveaway_dao.update_campaign_rules(
                data.campaign_id,
                rules_text,
            )
            if not campaign:
                raise ValueError("Giveaway campaign not found")
            await self.uow.commit()
        logger.info(f"{actor.log} Updated rules for giveaway campaign '{data.campaign_id}'")
        return campaign
    ...
    async def _execute(
        ...
        for campaign in campaigns:
            if campaign.id is None:
                continue
            for _ in range(10):
                code = generate_giveaway_code(campaign.code_prefix)
                async with self.uow:
...
@dataclass(frozen=True)
class AddManualGiveawayEntryDto:
    campaign_id: int
    phone: str


class AddManualGiveawayEntry(
    Interactor[AddManualGiveawayEntryDto, GiveawayEntryDto]
):
    required_permission = Permission.REMNASHOP_GIVEAWAY_EDITOR
    ...
    async def _execute(
        self,
        actor: UserDto,
        data: AddManualGiveawayEntryDto,
    ) -> GiveawayEntryDto:
        phone = data.phone.strip()
        if not phone.isdigit() or not 10 <= len(phone) <= 15:
            raise ValueError("Invalid phone")

        campaign = await self.giveaway_dao.get_campaign(data.campaign_id)
        if not campaign:
            raise ValueError("Giveaway campaign not found")
        if campaign.status not in {
            ...
            raise ValueError("Manual entries cannot be added to this campaign")

        plan = await self.plan_dao.get_by_id(campaign.eligible_plan_id)
        if not plan:
            raise ValueError("Giveaway campaign plan not found")

        for _ in range(10):
            async with self.uow:
                entry = await self.giveaway_dao.add_manual_entry(
                    GiveawayEntryDto(
                        campaign_id=data.campaign_id,
                        user_telegram_id=None,
                        telegram_username=None,
                        participant_code=generate_giveaway_code(campaign.code_prefix),
                        transaction_payment_id=None,
                        plan_id=campaign.eligible_plan_id,
                        plan_name=plan.name,
                        duration_days=campaign.eligible_duration_days,
                        purchase_type=None,
                        entry_source=GiveawayEntrySource.MANUAL,
                        phone=phone,
                    )
                )
                if entry:
                    await self.uow.commit()
                    logger.info(
                        f"{actor.log} Added manual giveaway entry id='{entry.id}' "
                        f"campaign='{data.campaign_id}' phone='{mask_giveaway_phone(phone)}' "
                        f"code='{entry.participant_code}'"
                    )
                    return entry
                await self.uow.rollback()

        logger.error(
            f"{actor.log} Failed to generate a unique manual giveaway code "
            f"for campaign='{data.campaign_id}' phone='{mask_giveaway_phone(phone)}'"
        )
        raise RuntimeError("Failed to generate unique giveaway participant code")
...
class SaveGiveawayPhone(Interactor[SaveGiveawayPhoneDto, GiveawayEntryDto]):
    ...
    async def _execute(self, actor: UserDto, data: SaveGiveawayPhoneDto) -> GiveawayEntryDto:
        ...
        async with self.uow:
            ...
            await self.uow.commit()
        masked = mask_giveaway_phone(data.phone)
