...
from src.application.events.system import TrialActivatedEvent
...
class ActivateTrialSubscription(Interactor[ActivateTrialSubscriptionDto, None]):
    ...
    async def _execute(self, actor: UserDto, data: ActivateTrialSubscriptionDto) -> None:
        ...
        try:
            ...
            logger.warning(
                f"{actor.log} Trial subscription grant skipped: "
                f"user '{user.telegram_id}' already exists in the panel"
            )
            raise
