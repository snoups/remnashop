...
In this guide we will be using Caddy as a reverse proxy to access Remnashop. We will point a domain name to our server and configure Caddy. Caddy will handle the issuance of SSL certificates by itself.
...
You can set up Caddy on the same server as the Remnawave panel or on a separate server. Select the appropriate option below.

<Columns cols={2}>
    <Card
        title="📄️ Bundled"
        href="/docs/en/reverse-proxies/caddy/bundled"
        arrow="true"
        cta="Go"
    >
        Bundled setup means Caddy will be configured on the same server as the
        Remnawave panel.
    </Card>
    <Card
        title="📄️ Separate server"
        href="/docs/en/reverse-proxies/caddy/separate-server"
        arrow="true"
        cta="Go"
    >
