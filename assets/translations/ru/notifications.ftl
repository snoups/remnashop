...
ntf-promocode =
    .not-found = ❌ <i>Промокод не найден.</i>
    .inactive = ❌ <i>Промокод неактивен.</i>
    .expired = ❌ <i>Срок действия промокода истёк.</i>
    .limit-exceeded = ❌ <i>Лимит активаций промокода исчерпан.</i>
...
ntf-giveaway =
    .registered =
        🎁 <b>Вы стали участником акции!</b>

        Ваш уникальный номер:
        <code>{ $code }</code>

        Сохраните этот номер до окончания розыгрыша.

        Если хотите, можете оставить контактный номер телефона, чтобы мы быстрее связались с вами в случае выигрыша.

        Формат номера: <code>8924830022332</code>
    ...
    .phone-request =
        ...
        Без плюса, пробелов, скобок и дефисов.

    .phone-invalid =
        Номер указан неверно.

        Отправьте номер только цифрами, без плюса и пробелов.

        Пример: <code>8924830022332</code>

    .phone-saved = Спасибо, контактный номер сохранён ✅
    .purchase-type-required = ❌ Выберите хотя бы один тип покупки.
    .admin-created = ✅ Акция создана.
    .status-updated = ✅ Статус акции обновлён.
    .winner-selected = 🎉 Победитель выбран.
    .winner-unavailable = ⚠️ Нельзя выбрать следующего победителя: нет доступных участников или все места уже заняты.
    .completed = ✅ Акция завершена.
    .archived = ✅ Акция и её участники архивированы.
    .deleted = Акция удалена ✅
    .rules-too-long = ❌ Текст условий не должен превышать 4000 символов.
    .rules-updated = ✅ Условия акции обновлены.
    .manual-phone-invalid =
        Номер указан неверно.
        ...
        Пример:
        <code>8924830022332</code>
    .manual-entry-unavailable = В завершённую или архивную акцию нельзя добавить участника.
    .buy-instruction =
        Для участия в акции выберите тариф:
        <b>{ $plan_name }</b>

        Срок: <b>{ $duration_days } дней</b>

        После успешной оплаты бот автоматически выдаст номер участника.
...
ntf-devices =
    ...
    .unavailable = ⚠️ <i>Не удалось выполнить операцию с устройствами. Обратитесь в поддержку.</i>
