...
msg-importer-main =
    ...
    На данный момент можно импортировать только пользователей (без поддержки реферальной системы).

    Выберите проект, из которого хотите произвести импорт.
...
msg-importer-from-bot =
    <b>📥 Импорт пользователей ({ $bot })</b>
